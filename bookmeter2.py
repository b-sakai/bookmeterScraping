...
def get_all_data(urlBase):
    ...
    while True:
        ...
        if len(datelist) == 0 or len(pagelist) == 0:
            break
    return allPageList, allDateList


def plot_cumulative_data(page_numbers, dates):
    # 不明な日付のインデックスを特定
    ...
    unknown_num = len(unknown_date_indices) + 1
    cumulative_book_counts = list(range(unknown_num, len(cumulative_page_numbers) + unknown_num))
    ...
    ax1.fill_between(dates, 0, cumulative_page_numbers, alpha=0.5, color='b')  # 修正：fill_betweenで塗りつぶし
    ...
    ax2 = ax1.twinx()
    ax2.scatter(dates, cumulative_book_counts, label='Cumulative Books', marker='x', color='r', alpha=0.2)
    ax2.fill_between(dates, 0, cumulative_book_counts, alpha=0.2, color='r')  # 修正：fill_betweenで塗りつぶし
...
if __name__ == '__main__':
    ...
    allPageList, allDateList = get_all_data(urlBase)
    allPageList.reverse()
    allDateList.reverse()
    plot_cumulative_data(allPageList, allDateList)
